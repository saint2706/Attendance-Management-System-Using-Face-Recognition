...
def verify_views_exceptions():
    ...
    checks = {
        "ValueError for DeepFace": "except ValueError as exc:" in content and "# DeepFace raises ValueError" in content,
        "AttributeError for DeepFace": "except AttributeError as exc:" in content and "# DeepFace raises AttributeError" in content,
        "OSError for DeepFace": "except OSError as exc:" in content and "# DeepFace raises OSError" in content,
        "OSError for cache": "except OSError as exc:  # pragma: no cover - defensive file I/O" in content,
...
def verify_webcam_exceptions():
    ...
    checks = {
        "OSError/RuntimeError for camera start": "except (OSError, RuntimeError) as exc:" in content and "# OSError: Camera device not available" in content,
        "OSError/RuntimeError for camera stop": content.count("except (OSError, RuntimeError) as exc:") >= 2,
        "Fallback Exception catches": content.count("except Exception as exc:  # pragma: no cover - catch unexpected errors") >= 2,
    }
