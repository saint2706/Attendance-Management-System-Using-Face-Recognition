...
pytestmark = [pytest.mark.ui, pytest.mark.accessibility, pytest.mark.django_db(transaction=True)]
...
def test_focus_returns_to_capture_button_after_try_again(
    ...
    page_with_camera.route("**/api/v1/attendance/mark/", lambda route: route.fulfill(
        status=200,
...
def test_focus_returns_to_capture_button_after_mark_another(
    ...
    # Wait for processing and result to appear
    mark_another_button = page_with_camera.get_by_role("button", name="Mark attendance for another person")
...
def test_focus_on_result_card_when_result_appears(
    page_with_camera: Page, server_url: str
) -> None:
...
def test_escape_key_triggers_reset(
    page_with_camera: Page, server_url: str
) -> None:
...
def test_space_key_shortcut_is_documented(
    page_with_camera: Page, server_url: str
) -> None:
