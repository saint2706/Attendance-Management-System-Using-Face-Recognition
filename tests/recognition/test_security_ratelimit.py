    ...
    with patch('recognition.tasks.train_recognition_model.delay') as mock_task:
        ...
        for i in range(4):
            ...
            if i < 3:
                ...
                assert any("Too many training requests" in str(m) for m in messages), "Rate limit message should be shown"
                return  # Test passed!
    ...
    with patch('recognition.tasks.capture_dataset.delay') as mock_task:
        ...
        for i in range(11):
            ...
            if i < 10:
                ...
                # Check if messages contain rate limit error
                messages = list(response.wsgi_request._messages)
                assert any("Too many capture requests" in str(m) for m in messages), "Rate limit message should be shown"
                return  # Test passed!
