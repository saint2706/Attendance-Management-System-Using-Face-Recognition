...
def test_n_plus_one_hours_vs_employee_given_date(django_assert_num_queries):
    ...
    for i in range(5):
        ...
        Time.objects.create(
            user=u, date=date, time=t1, direction=Direction.IN
        )
        Time.objects.create(
            user=u, date=date, time=t2, direction=Direction.OUT
        )
...
def test_n_plus_one_hours_vs_date_given_employee(django_assert_num_queries):
    ...
    for i in range(days):
        ...
        t2 = t1 + datetime.timedelta(hours=8)
        Time.objects.create(
            user=user, date=d, time=t1, direction=Direction.IN
        )
        Time.objects.create(
            user=user, date=d, time=t2, direction=Direction.OUT
        )
