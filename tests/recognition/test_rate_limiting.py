...
def _exercise_rate_limit(client, settings, url_name: str) -> tuple[int, int]:
    ...
    with patch(
        "recognition.views._mark_attendance", return_value=HttpResponse("ok")
    ) as mocked:
        for _ in range(2):
...
def test_mark_attendance_in_rate_limit_blocks_after_threshold(
    client, django_user_model, settings
):
    user = django_user_model.objects.create_user("rate", password="password")
    client.force_login(user)

    call_count, status_code = _exercise_rate_limit(
        client, settings, "mark-your-attendance"
    )
...
def test_mark_attendance_out_rate_limit_blocks_after_threshold(
    client, django_user_model, settings
):
    ...
    call_count, status_code = _exercise_rate_limit(
        client, settings, "mark-your-attendance-out"
    )
