...
def test_fairness_audit_config_defaults(tmp_path, monkeypatch):
    ...
    config = FairnessAuditConfig(reports_dir=tmp_path / "reports", dataset_root=tmp_path / "dataset")
...
def test_resolve_user_contexts_db():
    ...
    RecognitionAttempt.objects.create(username="employee_a", successful=True, site="hq", source="webcam")
    RecognitionAttempt.objects.create(username="employee_a", successful=True, site="hq", source="mobile")
    RecognitionAttempt.objects.create(username="employee_a", successful=True, site="branch", source="webcam")
    ...
    metrics = {
        "metrics_by_lighting": GroupMetrics(
            ...
            rows=[
                ...
                {"group": "moderate_light", "samples": 5, "far": 0.50, "frr": 0.50}, # Ignored (samples < 10)
            ],
