...
4. **Set up environment variables:**
    ...
    - In `DJANGO_DEBUG` or test runs, the settings loader first looks for these keys in the environment or `.env`. If they are absent, a deterministic pair is generated once and cached in `.dev_encryption_keys.json` (ignored by git) so encrypted artifacts remain readable across local restarts. Set `DEV_ENCRYPTION_KEY_FILE` or `LOCAL_ENV_PATH` to override the cache or `.env` locations when needed; delete the cache file to rotate local development keys.
