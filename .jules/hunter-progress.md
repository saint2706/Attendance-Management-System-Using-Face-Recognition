...
- `.github/workflows/ci.yml`: Fixed CI failure caused by using real GitHub Secrets (`${{ secrets.DJANGO_SECRET_KEY }}`, etc.) in the testing pipeline by replacing them with dummy strings. Note: Data encryption keys must be valid Fernet keys (e.g., `ufkljjgdbIMsc4N4-cVeRTtBk8sM6rDl6q-FMpepe8g=`).
