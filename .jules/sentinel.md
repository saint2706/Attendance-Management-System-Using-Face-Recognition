...
**Issue:** Replaced unsafe `pickle` operations with secure `skops` library.
**Action:** Replaced `pickle.dumps` and `pickle.loads` with `skops.io.dumps` and `skops.io.loads(..., trusted=True)` in `recognition/tasks.py`, `recognition/views.py`, and `recognition/views_legacy.py`. Also changed `np.load` to use `allow_pickle=False` where possible to prevent insecure deserialization. Added fallbacks for backward compatibility with older models and indices that were saved securely but require the legacy `pickle` loader or `allow_pickle=True`.
**Impact:** Mitigated critical RCE vulnerabilities related to insecure deserialization. Ensures `svc.sav` models and `faiss_index` files cannot be trivially exploited if the storage backend is compromised, while preserving runtime integrity and backward compatibility.
