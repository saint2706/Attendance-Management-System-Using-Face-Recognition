...
## 2026-04-24 - Strengthened Password Hashing and Validation
...
## 2026-05-25 - Insecure Pickle Mitigation in np.load
**Vulnerability:** The `np.load()` call in `recognition/tasks.py` was missing the explicit `allow_pickle=False` parameter, posing an arbitrary code execution risk.
**Prevention:** Added `allow_pickle=False` to all `np.load` calls to enforce secure deserialization.
