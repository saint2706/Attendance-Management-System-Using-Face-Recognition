...
- Added dynamic `aria-label` to theme toggle button based on state (light/dark).\n- Added `type="button"` to the logout button to prevent accidental form submission behavior.\n- Added dynamic `aria-label` to the password visibility toggle in the Login form.
