...
def _load_dataset_index(
    ...
    return views_module._load_dataset_embeddings_for_matching(
        model_name,
        detector_backend,
        enforce_detection,
    )
...
def _infer_samples(
    ...
    *,
    ...
    for path in image_paths:
        ...
        embedding = views_module._get_or_compute_cached_embedding(
            path,
            model_name,
            detector_backend,
            enforce_detection,
...
def run_face_recognition_evaluation(config: EvaluationConfig) -> EvaluationSummary:
    ...
    views_module = _recognition_views()
    model_name = views_module._get_face_recognition_model()
    detector_backend = views_module._get_face_detection_backend()
    enforce_detection = views_module._should_enforce_detection()
    distance_metric = views_module._get_deepface_distance_metric()
